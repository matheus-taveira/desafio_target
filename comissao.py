...
def calcular_comissao_venda(valor: float) -> float:
...
def calcular_comissao_total(dados_json: str) -> dict:
    """Lê o JSON e calcula a comissão total por vendedor."""
    ...
    for venda in dados["vendas"]:
        ...
        valor = venda["valor"]
        comissao = calcular_comissao_venda(valor)
        
        comissoes_por_vendedor[vendedor] = comissoes_por_vendedor.get(vendedor, 0.0) + comissao

    return comissoes_por_vendedor

if __name__ == "__main__":
    resultado_comissoes = calcular_comissao_total(DADOS_JSON)

    print("--- Resultado da Comissão por Vendedor ---")
    for vendedor, comissao_total in resultado_comissoes.items():
        print(f"**{vendedor}**: R$ {comissao_total:.2f}")